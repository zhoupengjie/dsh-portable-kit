...
`,
  // 与 POSIX 版 start 对齐的两点:
  //   1) 服务跑在前台并阻塞。原来用 start /b 分离启动,关掉控制台后服务仍然
  //      活着、端口一直被占,下次启动直接 EADDRINUSE(实测踩过)。前台运行
  //      才能让 Ctrl-C 和关窗口把它一起带走。
  //   2) 先探测端口,已在运行就只开窗口;否则轮询就绪,而不是盲等固定秒数。
  // 「等就绪→开浏览器」交给后台自调用(--open-when-ready),这样它不占前台。
  // 探测复用包内 Node,不依赖 netstat(输出随系统语言变化,不可靠)。
  'start.cmd': `@echo off
setlocal
${CMD_ENV}
set "PORT=3080"
...
if /I "%~1"=="--open-when-ready" goto :opener
