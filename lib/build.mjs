...
// 数一下已经落盘的包,用来给长时间安装做进度提示
async function countPackages(nodeModules) {
  let total = 0
  let entries
  try { entries = await readdir(nodeModules, { withFileTypes: true }) } catch { return 0 }
  for (const entry of entries) {
    if (!entry.isDirectory() || entry.name === '.bin') continue
    if (entry.name.startsWith('@')) {
      try { total += (await readdir(path.join(nodeModules, entry.name))).length } catch { /* 忽略 */ }
    } else total += 1
  }
  return total
}

// npm 7 之后取消了安装进度条,装 450+ 个包时它几乎不输出任何东西;Windows 上再
// 叠加 Defender 扫描,能静默二十分钟,看起来完全像卡死(实测误判过)。这里改用
// 异步 spawn —— spawnSync 会阻塞事件循环,连定时器都跑不了 —— 每 20 秒报一次进度。
//
// 两个信号缺一不可:解析/下载阶段 node_modules 一直是 0,只有缓存在长;落盘阶段
// 反过来。只报包数的话,健康的构建和真卡死的构建都显示「resolving, 0 packages」,
// 分不出来 —— 实测排查一次假死时,正是靠缓存有没有增长才判断出来的。
async function runWithProgress(label, command, args, options, watchDir, cacheDir) {
  const started = Date.now()
  let ticking = false
  const timer = setInterval(async () => {
    if (ticking) return // 上一次统计还没跑完就跳过,避免堆积
    ticking = true
    try {
      const mins = ((Date.now() - started) / 60000).toFixed(1)
      const [packages, cached] = await Promise.all([
        countPackages(watchDir),
        dirSize(cacheDir).catch(() => 0),
      ])
      const cache = cached > 0 ? `cache ${humanSize(cached)}` : 'cache empty'
      detail(packages > 0
        ? `${label}: ${packages} packages unpacked, ${cache}, ${mins} min elapsed`
        : `${label}: resolving/downloading, ${cache}, ${mins} min elapsed`)
    } finally {
      ticking = false
    }
  }, 20000)
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      child.on('close', (code) => (code === 0 ? resolve() : reject(new Error(`${command} failed (exit code ${code})`))))
    })
  } finally {
    clearInterval(timer)
  }
...
async function buildFatBase(opts) {
  ...
  const npm = (args, label) => runWithProgress(label, hostNode, [npmCli, ...args], {
    cwd: base,
    env: {
      ...process.env,
  ...
  }, path.join(base, 'node_modules'), path.join(ROOT, '.cache', 'npm'))
  ...
  detail('npm prints nothing while it works; progress is reported below every 20s')
  await npm(['install', '--ignore-scripts', '--no-audit', '--no-fund'], 'install')
  ...
  if (families.size > 0) {
    ...
    await npm(['install', '--ignore-scripts', '--no-audit', '--no-fund', '--force'], 'variants')
  }
...
function run(cmd, args, options) {
  ...
  if (r.status !== 0) throw new Error(cmd + ' failed (exit code ' + r.status + ')')
}
...
async function create(name) {
  ...
  if (existsSync(DSH_HOME)) {
    run('tar', ['-czf', path.join(dest, 'dsh-home.tar.gz'), '-C', path.dirname(DSH_HOME),
      // node_modules 一律不入档 —— 能从 profile 清单离线重建。
      // 通配必须写 */node_modules:pnpm 的 hoisted 布局把依赖放在 profiles/node_modules,
      // 而不是 profiles/<名字>/node_modules,写死后者根本排除不掉。漏排除在 Windows 上是
      // 灾难 —— 那 400 多个条目是 junction,tar 不当它们是符号链接而是直接钻进去,把
      // app/node_modules 的 3 万个文件反复打包(实测静默跑满 3 分钟,插件命令一步没走)。
...
if [ "\${DSH_AUTO_SNAPSHOT:-1}" != 0 ] && is_mutating "$@"; then
  echo "Snapshotting before plugin change... (set DSH_AUTO_SNAPSHOT=0 to skip)" >&2
...
goto :forward
:snap
echo Snapshotting before plugin change... ^(set DSH_AUTO_SNAPSHOT=0 to skip^) 1>&2
"%NODE%" "%ROOT%\launcher\snapshot.mjs" auto >nul 2>&1 && echo Snapshot done 1>&2
:forward
"%NODE%" "%ENTRY%" %*
`,
  'dsh-snap.cmd': String.raw`@echo off
setlocal
${CMD_ENV}
...
`,
  'dsh-restore.cmd': String.raw`@echo off
setlocal
${CMD_ENV}
"%NODE%" "%ROOT%\launcher\snapshot.mjs" restore %1
`,
  'pnpm.cmd': String.raw`@echo off
setlocal
set "ROOT=%~dp0"
if "%ROOT:~-1%"=="\" set "ROOT=%ROOT:~0,-1%"
...
async function assemble(target, base, opts, dshVersionActual) {
  ...
  // 只清掉每次都重新生成的部分。data/、workspace/、snapshots/、port.txt 是用户资产:
  // 升级 DSH 就是重新构建到同一个目录,整目录 rm 会把会话、设置、凭据、插件和快照
  // 一起删掉 —— 而这正是快照功能想防的事。两套启动器名字都列上,方便同一目录换目标。
  const regenerated = ['app', 'runtime', 'launcher', 'VERSION.json',
    'README.md', 'README_CN.md', 'README.txt',
    ...Object.keys(LAUNCHERS_SH), ...Object.keys(LAUNCHERS_CMD)]
  await mkdir(stage, { recursive: true })
  for (const entry of regenerated) {
    await rm(path.join(stage, entry), { recursive: true, force: true })
  }
